...
The **Sust. Standard Sales Invoice** report extends the standard sales invoice with sustainability data. It adds a CO2e-per-unit column to each invoice line, a total CO2e figure to the totals section, and a configurable disclaimer text pulled from the sustainability disclaimer setup for posted sales invoices. The emission values are formatted according to the decimal places and unit of measure defined in the Sustainability Setup, so the invoice layout matches the company's chosen emission reporting standard.

You can filter the report by the same criteria as the underlying Standard Sales - Invoice report, such as customer, invoice number, and posting date, since this extension does not introduce additional filters of its own; it only adds sustainability columns and totals to whichever invoices are selected.
...
* Check that emission figures use the unit of measure and decimal precision defined in the Sustainability Setup.,Accounts receivable and sales teams can use the report to:
