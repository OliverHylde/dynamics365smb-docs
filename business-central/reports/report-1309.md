...
The **Standard Sales - Return Rcpt.** report prints a formatted document for one or more posted return receipts. It shows the company and customer addresses, document dates, and the return order number, followed by a list of the returned items with quantities and units of measure. The report can be rendered in RDLC or Word layouts and supports options to log the interaction with the customer contact and to hide lines with zero quantity.

You can filter the report by fields such as No., Sell-to Customer No., and No. Printed on the Posted Return Receipt table, letting you select a specific return receipt, restrict output to a single customer, or limit results to receipts that have or haven't been printed before. A filter must be applied before running the report, so you avoid accidentally printing all posted return receipts at once.
...
* Send a customer a copy of the return receipt confirming which items and quantities were received back.
* Reprint a return receipt for a specific customer by filtering on Sell-to Customer No.
* Enable the Log Interaction option to record that the receipt was sent to the customer contact.,Warehouse and shipping staff can use the report to:

* Verify posted return quantities against physically received goods using the item lines on the receipt.
