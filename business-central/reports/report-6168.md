---
title: PostedSalesCrdMemoWithQR (report)
description: Add a QR code and E-Document layout support to the Standard Sales - Credit Memo report for posted sales credit memos.
author: kennieNP
ms.author: kepontop
...
# PostedSalesCrdMemoWithQR (report)
...
You can filter the report by the same filters available on the base **Standard Sales - Credit Memo** report, such as customer, credit memo number, and posting date, since this extension does not add new filter fields but only appends the QR code column and layout options to the existing dataset.
...
* Confirm that customer address, salesperson, applies-to document, and due date print correctly alongside the QR code before sending to customers.,Finance and compliance teams can use the report to:
...
* Review item line details—quantity, unit price, discount percentage, VAT percentage, and line amount—together with VAT-inclusive totals for audit purposes.
